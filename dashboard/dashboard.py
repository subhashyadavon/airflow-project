...
st.set_page_config(page_title="Job Market Insights Pro", layout="wide", initial_sidebar_state="expanded")
...
if not df_jobs.empty:
    ...
    selected_roles = st.sidebar.multiselect("Select Job Roles", options=all_roles, default=all_roles)
    
    # Filter Data
    filtered_jobs = df_jobs[df_jobs['role'].isin(selected_roles)]
    filtered_merged = df_merged[df_merged['role'].isin(selected_roles)] if not df_merged.empty else pd.DataFrame()
    ...
    with m2:
        ...
        st.metric("Average Salary", f"${avg_sal:,.0f}" if not pd.isna(avg_sal) else "N/A")
    with m3:
        unique_skills = filtered_merged['skill_name'].nunique() if not filtered_merged.empty else 0
    ...
    with col_a:
        ...
        chart_roles = (
            ...
            .encode(
                ...
                color=alt.Color(field="role", type="nominal", scale=alt.Scale(scheme="category20b")),
                tooltip=["role", "count"]
            )
    ...
    with col_b:
        ...
        chart_sal_role = (
            ...
            .encode(
                ...
                color=alt.Color("avg_salary:Q", scale=alt.Scale(scheme="magma"), legend=None),
    ...
    with col1:
        ...
        if not filtered_merged.empty:
            skill_demand = filtered_merged.groupby('skill_name').size().reset_index(name='count')
            skill_demand['percentage'] = (skill_demand['count'] / len(filtered_jobs) * 100).round(1)
            ...
            chart_demand = (
                ...
                .encode(
                    ...
                    color=alt.Color("percentage:Q", scale=alt.Scale(scheme="viridis"), legend=None),
                    tooltip=["skill_name", "count", "percentage"]
                )
            ).properties(height=400)
            st.altair_chart(chart_demand, use_container_width=True)
        else:
            st.info("No specific technical skills detected in the current job descriptions for these roles.")

    with col2:
        st.subheader("💰 Highest Paying Skills")
        if not filtered_merged.empty:
            skill_salary = filtered_merged.groupby('skill_name')['avg_salary'].agg(['mean', 'count']).reset_index()
            ...
            chart_salary = (
                ...
                .encode(
                    ...
                    y=alt.Y("skill_name:N", sort="-x", title="Skill"),
                    color=alt.Color("mean:Q", scale=alt.Scale(scheme="magma"), legend=None),
    ...
    if not filtered_merged.empty:
        ...
        st.info(f"💡 **Market Insight:** For the selected roles, **{top_skill}** is the most critical skill, appearing in {skill_demand.iloc[0]['percentage']}% of job listings.")

else:
    st.warning("⚠️ No job data found in the database.")
    st.info("1. Go to Airflow [http://localhost:8080](http://localhost:8080)\n2. Unpause and Trigger `daily_job_pipeline`.")

# Data Table
with st.expander("📂 Explore Raw Job Postings"):
    st.dataframe(filtered_jobs if not df_jobs.empty else df_jobs, use_container_width=True)
