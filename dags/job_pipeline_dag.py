...
def extract_jobs_task(**kwargs):
    ...
    jobs = extractor.fetch_jobs(what="software engineer", where="Winnipeg")
    kwargs['ti'].xcom_push(key='raw_jobs', value=jobs)
    return jobs
...
def data_quality_check_task(**kwargs):
    ...
    for job in cleaned_jobs:
        if not job.get('title'):
            raise ValueError(f"Data Quality Check FAILED: Job {job.get('job_id')} has no title.")
        
        # Check salary range if available
        avg_salary = job.get('avg_salary')
        if avg_salary is not None:
            if avg_salary < 10000 or avg_salary > 500000:
                logging.warning(f"Data Quality Alert: Unusual salary for job {job.get('job_id')}: {avg_salary}")
